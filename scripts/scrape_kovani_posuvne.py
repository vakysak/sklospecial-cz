#!/usr/bin/env python3
"""Build wp-theme/sklospecial/assets/data/kovani-posuvne.json from luxusnikovani.cz.

Source: /kovani-pro-sklenene-posuvne-dvere (+ subcategories, pagination).
Applies +35 % margin rounded to tens Kč (same as kování sprchy / zábradlí / stříšky).
"""
...
OUT = ROOT / "wp-theme" / "sklospecial" / "assets" / "data" / "kovani-posuvne.json"
UA = "Mozilla/5.0 (compatible; SklospecialBot/1.0; +https://sklospecial.eu)"
BASE = "https://www.luxusnikovani.cz"
SECTION = "kovani-posuvne"
MARKUP = 1.35
CATEGORY_URL = BASE + "/kovani-pro-sklenene-posuvne-dvere"

# Zámky first: some lock SKUs also appear under mušle; first-seen wins.
ROOT_SUBS = [
    "/zamky-pro-posuvne-sklenene-dvere",
    "/musle-a-uchytky-pro-sklenene-posuvne-dvere",
    "/pojezdy-pro-sklenene-posuvne-dvere",
]

# Czech labels from luxusnikovani.cz /kovani-pro-sklenene-posuvne-dvere (slug → label)
...
def make_code(url: str, used: set[str]) -> str:
    ...
    if len(raw) > 40:
        raw = raw[:40].rstrip("-")
    code = f"KOV-POS-{raw}" if raw else "KOV-POS-X"
    base = code
    n = 2
    while code in used:
        code = f"{base}-{n}"
