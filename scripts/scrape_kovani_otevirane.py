#!/usr/bin/env python3
"""Build wp-theme/sklospecial/assets/data/kovani-otevirane.json from luxusnikovani.cz.

Source: /kovani-pro-sklenene-dvere (+ subcategories, pagination).
Applies +35 % margin rounded to tens Kč (same as kování posuvné / sprchy / zábradlí / stříšky).
"""
...
OUT = ROOT / "wp-theme" / "sklospecial" / "assets" / "data" / "kovani-otevirane.json"
UA = "Mozilla/5.0 (compatible; SklospecialBot/1.0; +https://sklospecial.eu)"
BASE = "https://www.luxusnikovani.cz"
SECTION = "kovani-otevirane"
MARKUP = 1.35
CATEGORY_URL = BASE + "/kovani-pro-sklenene-dvere"

# Supplier order from /kovani-pro-sklenene-dvere (first-seen wins on duplicate SKUs).
ROOT_SUBS = [
    "/kliky-a-stitky-pro-sklenene-dvere",
    "/tectus-glass-skryty-pant-pro-sklo",
    "/hydraulicke-panty-simonswerk-pro-sklenene-dvere",
    "/zavesy-pro-sklenene-dvere",
    "/koule-a-uchyty-pro-sklenene-dvere",
    "/automaticke-padaci-listy-pro-sklenene-dvere",
    "/madla-pro-sklenene-dvere",
    "/elektronicky-zamek-pro-sklenene-dvere",
    "/doplnky-pro-sklenene-dvere",
]

# Czech labels from luxusnikovani.cz /kovani-pro-sklenene-dvere (slug → label)
...
def make_code(url: str, used: set[str]) -> str:
    ...
    if len(raw) > 40:
        raw = raw[:40].rstrip("-")
    code = f"KOV-OTE-{raw}" if raw else "KOV-OTE-X"
    base = code
    n = 2
    while code in used:
        code = f"{base}-{n}"
...
def list_all_products() -> list[dict]:
    products: list[dict] = []
    seen: set[str] = set()
    # (path, root_subcat_slug) — nested listings keep the top-level chip/section
    queue: list[tuple[str, str]] = [(p, p.strip("/")) for p in ROOT_SUBS]
    visited: set[str] = set()
    while queue:
        path, root_sub = queue.pop(0)
        if path in visited:
            continue
        visited.add(path)
        url = abs_url(path)
        ...
        while url and page < 40:
            page += 1
            print(f"  list {path} [{root_sub}] p{page}")
            html = fetch(url)
            kids = parse_subcats(html)
            batch = parse_listing_page(html, root_sub)
            if kids and not batch:
                for _title, href in kids:
                    if href not in visited:
                        queue.append((href, root_sub))
                break
            added = 0
            for p in batch:
                if p["url"] in seen:
